...
static double tempk[][][];
static double m[][];
static double diff[];git reset --hard HEAD~~
...
LinkedList<LinkedList> d = new LinkedList<>();
LinkedList<LinkedList<LinkedList>> k = new LinkedList<>();
LinkedList<LinkedList<LinkedList>> tempk = new LinkedList<>();
LinkedList<LinkedList> m = new LinkedList<>();
static double diff[];
static int n,p;
...
// }
static double distance(LinkedList x, LinkedList y){
    return Math.sqrt((x.get(0)-y.get(0))*(x.get(0)-y.get(0))+(x.get(1)-y.get(1))*(x.get(1)-y.get(1)) ) ;   
}
...
//     return val;
// }
static int cal_diff(LinkedList a) 
{
    int temp1=0;
    for(int i=0;i<p;++i)
    {
        diff.get(i)=distance(a,m.get(i));
    }
    int val=0;
    double temp=diff.get(0);
        for(int i=0;i<p;++i){
        if(diff.get(i)<temp){
            temp=diff.get(i);
            val=i;
